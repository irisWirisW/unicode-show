...
export class UnicodeInfoService {
    ...
    static getCharacterInfo(char: string, codePoint: number, unicodeHex: string): UnicodeCharacterInfo {
        const name = this.getUnicodeName(codePoint);
        const category = this.getUnicodeCategory(codePoint);
        const script = this.getUnicodeScript(codePoint);
        ...
        const info: UnicodeCharacterInfo = {
            ...
            script,
            utf8Bytes: this.getUTF8Bytes(char),
            utf16Bytes: this.getUTF16Bytes(char),
    ...
    private static getCategoryDescription(category: string): string {
        return this.CATEGORY_DESCRIPTIONS[category] || category;
    }

    /**
     * 备用方案：基于码点范围获取Unicode信息
     */
    private static getUnicodeInfoFallback(codePoint: number): { name: string; category: string; script: string } {
    ...
    private static getUnicodeName(codePoint: number): string {
        try {
            if (unicode && unicode.getName) {
                return unicode.getName(codePoint) || this.getUnicodeInfoFallback(codePoint).name;
            }
        } catch (e) {
            console.warn('Error getting Unicode name:', e);
        }
        return this.getUnicodeInfoFallback(codePoint).name;
    }
    ...
    private static getUnicodeCategory(codePoint: number): string {
        try {
            if (unicode && unicode.getCategory) {
                return unicode.getCategory(codePoint) || this.getUnicodeInfoFallback(codePoint).category;
            }
        } catch (e) {
            console.warn('Error getting Unicode category:', e);
        }
        return this.getUnicodeInfoFallback(codePoint).category;
    }
    ...
    private static getUnicodeScript(codePoint: number): string {
        try {
            if (unicode && unicode.getScript) {
                return unicode.getScript(codePoint) || this.getUnicodeInfoFallback(codePoint).script;
            }
        } catch (e) {
            console.warn('Error getting Unicode script:', e);
        }
        return this.getUnicodeInfoFallback(codePoint).script;
    }
}
